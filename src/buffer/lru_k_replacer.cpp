...
namespace bustub {
    ...
    auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
        latch_.lock();
        size_t inf_earliest_timestamp = SIZE_T_MAX, earliest_timestamp = SIZE_T_MAX;
        for (size_t fid = 0; fid < this->replacer_size_; fid += 1) {
            ...
            if (this->evictable_frames_[fid]) {
                if (!records.empty()) {
                    ...
                    if (records.size() == this->k_ && inf_earliest_timestamp == SIZE_T_MAX && records.front() < earliest_timestamp) {
        ...
        if (inf_earliest_timestamp != SIZE_T_MAX || earliest_timestamp != SIZE_T_MAX) {
            latch_.unlock();
    ...
    void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
        ...
        size_t t = std::chrono::duration_cast<std::chrono::nanoseconds>(
